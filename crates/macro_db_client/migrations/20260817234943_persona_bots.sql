...
-- A session may now run without a checkout, so the repo it resolved to is
-- genuinely optional. It stays a recorded fact per session (rather than being
-- read back off the persona) so editing a persona never rewrites the history
-- of what a past session actually cloned.
ALTER TABLE public.agent_session ALTER COLUMN repo_url DROP NOT NULL;
